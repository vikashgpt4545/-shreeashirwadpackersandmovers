...
$faq_list = [
    ...
    [
        'q' => 'How long does shipment transit take from Chennai to Tirunelveli?',
    ...
    [
        'q' => 'How are relocation charges calculated for Bike Transport from Chennai to Tirunelveli | Safe Two-Wheeler Shifting?',
        'a' => 'Our moving estimates between Chennai and Tirunelveli are calculated using transparent parameters: total consignment volume (cft), distance (~650 km), packaging material grade (7-layer corrugated and bubble sheets), dedicated enclosed container capacity, and insurance declaration. We provide 100% itemized, fixed-price quotations with zero moving-day surcharges.'
    ],
    [
    ...
    [
        ...
        'a' => 'Yes. Upon vehicle dispatch from Chennai, you receive a private GPS tracking portal link and automated WhatsApp transit checkpoints as our truck clears interstate toll plazas and border checkposts en route to Tirunelveli.'
    ],
];
...
We need to heal the section. Remove AI thought leaks etc. Ensure no stray backticks. Replace placeholder tokens: none. Convert any raw markdown tables? None. Ensure design system palette: background #070d1e etc. But we need to preserve dark navy palette (#0b132b or rgba(15,23,42,0.75)). The current background is #070d1e, which is close but not exactly. We can keep but maybe adjust to #0b132b. Also ensure color palette: text color #cbd5e1. Badges etc. The section currently has a badge with background rgba(59,130,246,0.12) color #60a5fa. That's fine.

We need to ensure no AI thought leaks. The content seems fine. But we need to check for any placeholder tokens: none.

Check for escaped HTML tags: none.

Check for out-of-context corridors: The content is about Chennai to Tirunelveli. It's fine.

Empty/hollow cards: None.

We need to en<section> contains <div class="container"> etc. The inner <div> has a <span> badge, <h2>, &lt;div&gt; with paragraph, grid of two cards, etc. At the end, there&#039;s &lt;h3&gt;Comparing Packers And Movers Companies</h3></div> but the closing tags: The <div> after <h3>Comparing Packers And Movers Companies</h3></div> closes the inner container? Let's check structure:
...
<p>It is crucial to compare...</p>

<h3>Additional Services</h3>
<p>...</p>
<ul>...</ul>

<p>When comparing providers...</p>
...
The <div> after <h3>Comparing Packers And Movers Companies</h3> closes the inner container that started after <div style="...">. But we need to ensure that the <div> that started after <div style="..."> encloses all content. That seems correct.

But we need to ensure that the <div> that started after <div style="..."> is closed before the outer container. It is closed by that </div>. Then the outer container <div class="container"> is closed by </div>. Then section closed.

But we need to check if any stray tags: The <div style="display: grid; ..."> is closed properly. The <div> inside each card closed. Good.

Now we need to ensure no stray backticks. None.

We need to ensure no AI thought leaks. The content is fine.

But we might want to add a small note: The section ends with <h3>Comparing Packers And Movers Companies</h3> but no content after that. That might be incomplete. We could add a short paragraph summarizing. But the instruction says if empty card, write 2-3 sentences. But this is not a card; it's a heading. We could add a paragraph after that heading to provide guidance. Let's add a paragraph: "When choosing a packer and mover, evaluate their experience with bike transport, insurance coverage, and customer support." That would fill.

Also we might want to adjust the badge style to match design system: maybe use 'Verified Shree Ashirwad Standard' tag. But the current badge is generic. We could change to "Verified Shree Ashirwad Standard" with same style. Let's modify the badge text accordingly.

Also we might want to adjust the background to #0b132b. Let's change the outer section background to #0b132b. The inner container background currently rgba(15,23,42,0.65). We can keep.

Also the badge background: rgba(59,130,246,0.12) color #60a5fa. That is fine.

Also we need to ensure the color palette: text color #cbd5e1. The inner
